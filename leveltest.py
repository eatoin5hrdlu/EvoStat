...
if __name__ == "__main__" :
    ...
    if ( 'iterate' in sys.argv ) : # Read lagoon levels
        debug = True
        plog("iterate: " + str(img.shape))
        brect = ipcam.params['lagoonRegion']
        quarterWidth = (brect[3]-brect[1])/4
        eighthWidth = quarterWidth/2
        plog("QUARTER width " + str(quarterWidth))
        ...
        for i in range(ipcam.params['numLagoons']) :
            ...
            ( (lx1,ly1), (lx2,ly2) ) = ipcam.lagoon_level_bbox_xy(i)
            lbb = (ly1,lx1,ly2,lx2)
            for cscale in [1.5,1.8,1.9,2.0,2.1] :
                for coffset in [-50,-60,-70,-90] :
                    for itrs in [1,2] :
                        llev = ipcam.evocv2.getLevel( img,
                                                      lbb,
                                                      green,
                                                      (itrs,cscale,coffset))
                        print("CONTRAST " + str((itrs,cscale,coffset)) + 
                              "LBB " + str(lbb) + " level " + str(llev))
        levelPhase = 1
        brect = ipcam.params['cellstatRegion']
        num = 1
        contrast = ipcam.params['cellstatContrast']
