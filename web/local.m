...
x = lsode("f2", [1000;0;0;p0], (t = linspace (0,hours,steps)' ));
y = lsode("g2", [1000;0;0;p0], (u = linspace (0,hours,steps)' ));
fhandle = figure(1);
if (lsl == 1)
  plot(t,x);
  hold on;
...
text(pos1,
     ...
     'Uninfected Host','fontsize',22,'color','k');
text(pos2,
     offset(x(leftp,2),lsl,x(leftp,3)),
     'Adsorbed Host',  'fontsize',24,'color',host2color);
text(pos2,
     offset(x(leftp,3),lsl,x(leftp,2)),
     'Productive Host','fontsize',20,'color',host3color);
text(pos1,
