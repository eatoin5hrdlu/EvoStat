...
#                print "usb.read() returned " + str(rval)
    ...
    def cellstatImage(self):
        (x1,y1,x2,y2) = ipcam.params['cellstatRegion']
        image = self.grab()
        if (image == None) :
            print "camera(fail)."
            exit(0)
        self.exportImage(image)
        return image[x1:x2,y1:y2,:] # cropped for cellstat
